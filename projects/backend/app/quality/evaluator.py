...
def _sentence_lengths(answer: str) -> List[int]:
    sentences = [segment.strip() for segment in answer.replace("?", ".").replace("!", ".").split(".") if segment.strip()]
    ...
    def evaluate(
        ...
        *,
        ...
        unique_doc_citations = {
            ...
            if citation.get("type") == "document" and citation.get("metadata", {}).get("document_id")
        }
        coverage_score = (
            1.0
            if doc_count == 0
    ...
    def _score_criterion(
        ...
        *,
        ...
        if criterion.key == "self_paced_scaffolding":
            has_steps = any(token.strip().startswith(tuple(str(i) + "." for i in range(1, 6))) for token in answer.splitlines())
